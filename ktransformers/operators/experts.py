...
class KExpertsCPU(KExpertsBase):
    ...
    def submit_for_one_decode(self, input_tensor, expert_ids, weights):
        ...
        self.cpu_infer.submit_with_cuda_stream(torch.musa.current_stream(self.out_device).musa_stream, self.moe.forward(1, expert_ids.size(0), KExpertsCPU.expert_ids_cpu.data_ptr(), KExpertsCPU.weights_cpu.data_ptr(), KExpertsCPU.input_tensor_cpu.data_ptr(), KExpertsCPU.output_cpu.data_ptr()))

    def sync_for_one_decode(self):
        self.cpu_infer.sync_with_cuda_stream(torch.musa.current_stream(self.out_device).musa_stream)
    ...
    def forward(self, input_tensor, expert_ids, weights):
        ...
        # print(expert_ids)
        if input_tensor.size(0)==1 and torch.musa.is_current_stream_capturing():
            ...
            KExpertsCPU.expert_ids_cpu.copy_(expert_ids, non_blocking=True)
            KExpertsCPU.weights_cpu.copy_(weights, non_blocking=True)
            self.cpu_infer.submit_with_cuda_stream(torch.musa.current_stream().musa_stream, self.moe.forward(1, expert_ids.size(1), KExpertsCPU.expert_ids_cpu.data_ptr(), KExpertsCPU.weights_cpu.data_ptr(), KExpertsCPU.input_tensor_cpu.data_ptr(), KExpertsCPU.output_cpu.data_ptr()))
            self.cpu_infer.sync_with_cuda_stream(torch.musa.current_stream().musa_stream)
            KExpertsCPU.output_gpu_map[self.out_device].copy_(KExpertsCPU.output_cpu, non_blocking=True)
            return KExpertsCPU.output_gpu_map[self.out_device]
        else:
...
class KDeepseekV2MoE(BaseInjectedModule, DeepseekV2MoE):
    def forward(self, hidden_states):
        ...
        if sequence_length == 1 and hasattr(self.experts.generate_experts, "submit_for_one_decode") and torch.musa.is_current_stream_capturing():
